...
internal partial class MessageManager
{
    private void ProcessTimeForbiddenMessage(TimeForbiddenMessage message)
    {
        ...
        this._monitor.Log($"Got Time Forbidden Message {message.Reason} from farmer {farmer.Name}", LogLevel.Alert);
    ...
    private void ProcessTimeManipulateMessage(TimeManipulateMessage message)
    {
        ...
        this._monitor.Log($"Got Time Manipulate Message {{ FreezeTimeMethod: {message.FreezeTimeMethod} | Increase: {message.Increase} }} from farmer {farmer.Name}", LogLevel.Alert);
    ...
    private void ProcessTimeManipulateForLocationMessage(TimeManipulateForLocationMessage message)
    {
        ...
        this._monitor.Log($"Got Time Manipulate For Location Message {{ FreezeTimeMethod: {message.FreezeTimeMethod} | Increase: {message.Increase} | GameLocation : {ModEntry.GetLocationFromID(message.Location)?.Name ?? "Unknown"} }} from farmer {farmer.Name}", LogLevel.Alert);
    ...
    private void ProcessTimeStateReplyMessage(TimeStateReplyMessage message)
    {
        ...
        this._monitor.Log($"Got Time State Reply Message {{ \"Message: {message.Message}\" | Timeout: {message.Timeout} }} from farmer {farmer.Name}", LogLevel.Alert);
    ...
    private void ProcessTimeConfigStateMessage(TimeConfigStateMessage message)
    {
        ...
        this._monitor.Log($"Got Time Config State Message {{ HostOnly: {message.HostOnly} | VoteEnabled: {message.VoteEnabled} | VoteThreshold: {message.VoteThreshold} }} from farmer {farmer.Name}", LogLevel.Alert);
    ...
    private void ProcessTimeInfoMessage(TimeInfoMessage message)
    {
        ...
        this._monitor.Log($"Got Time Info Message {{ HostOnly: \"{message.Message}\" }} from farmer {farmer.Name}", LogLevel.Alert);
    ...
    private void ProcessTimeVotePauseMessage(TimeVotePauseMessage message)
    {
        ...
        this._monitor.Log($"Sent Time Vote Pause Message {{ VoteCast: {message.VoteCast} | Finish: {message.Finish} }} from farmer {farmer.Name}", LogLevel.Alert);
